...
class CornersProblem(SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        # *** Your Code Here ***
        self._numExpanded = 0
    # function returns both the starting position along with list of corners visited

    def startingState(self):
        corners_visited = []
        return(self.startingPosition, corners_visited)

    # function checks if state is goal state through the list of corners visited

    def isGoal(self, state):
        if len(state[1]) != 4:
            return False
        return True
    # function returns list of successor states (state, action, cost)
    ...
    def successorStates(self, state):
        ...
        for action in Directions.CARDINAL:
            ...
            if not hitsWall:
                ...
                # if next state is in an unvisited corner, list of visited corners is updated
                if nextState in self.corners:
                    if nextState not in state[1]:
                        corners_visited = state[1] + [nextState]
                        # append the list of sucessors with updated corners list
                        successors.append(((nextState, corners_visited), action, 1))
                # append list of successors  without updating corners list
                else:
...
def cornersHeuristic(state, problem):
    ...
    # create list for corners visited and not visited and set heuristic to 0
    corners = problem.corners
    corners_visited = state[1]
    corners_not_visited = []
    heuristic_value = 0
    # add corners not visited to the not visited list
    for corner in corners:
        if corner not in corners_visited:
            corners_not_visited.append(corner)
    # repeat following until all corners have been visited
    while corners_not_visited:
        distance_list = []
        # calculate the estimated distance from state to each corner and store in list
        for corner in corners_not_visited:
            distance = euclidean(corner, state[0])
            distance_list.append(distance)
        # find the minimum estimated distance to a corner and add to heuristic
        min_distance = min(distance_list)
        heuristic_value += min_distance
        # remove the corner with the minimum estimated distance from the not visited list
